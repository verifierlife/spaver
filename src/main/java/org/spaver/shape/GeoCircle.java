...
public class GeoCircle extends Circle {
	...
	private void init() {
		if (radiusDEG > 90) {
			...
			// some rare numeric conditioning cases can cause this to be barely beyond the
			// box
	...
	protected SpatialRelation relateRectanglePhase2(Rectangle r, SpatialRelation bboxSect) {
		...
		if (cornersIntersect == 4) {
			// ensure r's x axis is within c's. If it doesn't, r sneaks around the globe to
			// touch the other side (intersect).
	...
	private SpatialRelation relateRectangleCircleWrapsPole(Rectangle r, SpatialContext ctx) {
		// This method handles the case where the circle wraps ONE pole, but not both.
		// For both, there is the inverseCircle case handled before now. The only exception is for
		// the case where the circle covers the entire globe, and we'll check that first.
		...
		// Check corners:
		int cornersIntersect = numCornersIntersect(r);
		// (It might be possible to reduce contains() calls within nCI() to exactly two,
	...
	public String toString() {
		...
		// instead of String.format() so that we get consistent output no matter the
		// locale
