...
public class BBoxCalculator {
	...
	/**
	 * Sorted list of <em>disjoint</em> X ranges keyed by maxX and with minX stored
	 * as the "value".
	 */
	private TreeMap<Double, Double> ranges; // maxX -> minX

	// note: The use of a TreeMap of Double objects is a bit heavy for the
	// points-only use-case. In such a case,
	// we could instead maintain an array of longitudes we just add onto during
	// expandXRange(). A simplified version
	// of the processRanges() method could be used that initially sorts and then
	// proceeds in a similar but simplified
	// fashion.
...
//		}
		...
		// Get an iterator starting from the first entry that either contains minX or
		// it's to the right of minX
		...
		if (rangeContains(entryMin, entryMax, maxX)) {
			// Easy: either minX is also within this entry in which case nothing to do, or
			// it's below in which case
			...
			if (rangeContains(entryMin, entryMax, minX)) {
				...
				// note: TreeMap's Map.Entry doesn't support setting the value :-( So we remove
				// & add the entry.
				entryIter.remove();
			...
			// We're going to insert an entry. Determine it's min & max. While finding the
			// max, we'll delete entries
			...
			Double newMaxX = maxX;
			// Loop through entries (starting with current) to see if we should remove it.
			// At the last one, update newMaxX.
	...
	private void processRanges() {
		if (ranges.size() == 1) { // an optimization
			...
			// Find the biggest gap. Whenever we do, update minX & maxX for the rect
			// opposite of the gap.
	...
	public boolean doesXWorldWrap() {
		//assert ctx.isGeo();
		// note: not dependent on "ranges", since once we expand to world bounds then
		// ranges is null'ed out
